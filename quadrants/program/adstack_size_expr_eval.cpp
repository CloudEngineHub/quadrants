...
int64_t evaluate_external_tensor_read(const SerializedSizeExprNode &node,
  ...
  // Resolve each index (possibly via a bound variable) then compose them into the C-order linear offset
  // `sum_i(idx_i * prod_{j>i}(shape_j))`. Multi-dim shapes are resolved by reading `arr.shape[axis]` off the
  // launch context (same `SHAPE_POS_IN_NDARRAY` path `ExternalTensorShape` uses), so an ndarray indexed by two
  // loop variables lowers to the correct element rather than a sum-of-indices garbage read.
  ...
  for (std::size_t i = node.indices.size(); i > 0; --i) {
    ...
    if (i - 1 > 0) {
      ...
      // Ndarray shapes live in the kernel's args struct as `int32` fields (see `TypeFactory::SHAPE_POS_IN_NDARRAY`
      // / `set_arg_ndarray_impl`), so read them with the matching `int32_t` template. Using `int64` steps 8 bytes
      // past the slot and sign-extends into the next field, producing a huge `stride` and an OOB deref.
      stride *= static_cast<int64_t>(ctx->get_struct_arg<int32_t>(sh_idx));
    }
  }
...
int32_t encode_subtree(const SerializedSizeExpr &src,
  ...
  switch (kind) {
    ...
    case SizeExpr::Kind::Mul:
    case SizeExpr::Kind::Max: {
      int32_t a = encode_subtree(src, node.operand_a, contains_device_leaf, free_vars, prog, ctx, fl_emitter, out_nodes,
                                 out_indices);
      int32_t b = encode_subtree(src, node.operand_b, contains_device_leaf, free_vars, prog, ctx, fl_emitter, out_nodes,
                                 out_indices);
    ...
    case SizeExpr::Kind::MaxOverRange: {
      ...
      int32_t b = encode_subtree(src, node.operand_b, contains_device_leaf, free_vars, prog, ctx, fl_emitter, out_nodes,
                                 out_indices);
      int32_t body = encode_subtree(src, node.body_node_idx, contains_device_leaf, free_vars, prog, ctx, fl_emitter,
    ...
    case SizeExpr::Kind::FieldLoad: {
      ...
      // element size to get the final byte offset.
      for (std::size_t a = 0; a < node.indices.size(); ++a) {
        out_indices.push_back(static_cast<int32_t>(node.indices[a]));
...
std::vector<uint8_t> encode_bytecode_common(std::vector<AdStackSizeExprDeviceStackHeader> stack_headers,
  ...
  for (std::size_t i = 0; i < n_stacks; ++i) {
    ...
    sh.root_node_idx = encode_subtree(*expr, static_cast<int32_t>(root_src_idx), contains_device_leaf, free_vars, prog,
                                      ctx, fl_emitter, nodes, indices);
  }
