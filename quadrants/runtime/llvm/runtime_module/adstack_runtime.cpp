...
// a constant and DCE's the entire post-loop atomic-reduce block. The recognizer restricts max-reducer body kinds
// to `kConst / kBoundVariable / kExternalTensorRead / kAdd / kSub / kMul / kMax` (no `kMaxOverRange` /
// `kFieldLoad`), so a fixed-size stack walk of the post-order body bytecode covers every accepted shape with no
// recursion. Stack depth is bounded by tree depth; recognized bodies are tiny (a few nodes), so 32 slots is
// plenty.
...
__attribute__((always_inline)) inline i64 device_eval_max_reduce_body_iterative(
  ...
  for (i32 idx = 0; idx < body_node_count; ++idx) {
    ...
    switch (static_cast<K>(node.kind)) {
      ...
      default:
        // Out-of-grammar kinds (kMaxOverRange / kFieldLoad) shouldn't appear in a recognizer-accepted body. Push
        // 0 so the kernel completes deterministically; the host's diagnose path re-runs the tree and raises.
        stack[sp++] = 0;
        break;
    }
  }
...
__attribute__((always_inline)) inline i64 runtime_eval_adstack_max_reduce_one_step(
  ...
  return device_eval_max_reduce_body_iterative(nodes, indices, body_node_count, scope, arg_buffer);
}
}  // namespace
...
extern "C" void runtime_eval_adstack_max_reduce_serial(LLVMRuntime *runtime,
  ...
  for (u64 i = 0; i < total_length; ++i) {
    i64 v = runtime_eval_adstack_max_reduce_one_step(nodes, indices, params->body_node_count, params, i, num_axes,
                                                     &scope, arg_buffer);
    if (v > running_max) {
      running_max = v;
    }
...
extern "C" void runtime_eval_adstack_max_reduce_parallel(LLVMRuntime *runtime,
  ...
  // Atomic-max reduction across all per-thread maxima into the host-seeded INT64_MIN slot. Backend split:
  //   - CUDA: inline-asm `atom.global.cas.b64` with explicit `cvta.to.global`. The pointer comes from a memory
  //     load (`runtime->adstack_max_reducer_outputs[output_slot]`) so the NVPTX backend can't statically prove
  //     the addrspace; an LLVM IR-level `atomicrmw` / `cmpxchg` is silently dropped in that case. Inline asm
  //     bypasses every backend heuristic.
  //   - AMDGPU: `__atomic_compare_exchange_n` CAS loop. Clang lowers it to AMDGPU's native `flat_atomic_cmpswap`
  //     equivalent without the NVPTX-specific addrspace-inference issue.
  if (running_max != (i64)0x8000000000000000ll) {
    ...
    while (running_max > old_val) {
      ...
      asm volatile(
          "{\n\t"
          ".reg .u64 %%global_ptr;\n\t"
          "cvta.to.global.u64 %%global_ptr, %1;\n\t"
          "atom.global.cas.b64 %0, [%%global_ptr], %2, %3;\n\t"
          "}\n\t"
          : "=l"(prev)
          : "l"(slot), "l"(old_val), "l"(running_max)
          : "memory");
      if (prev == old_val) {
        break;
      }
