...
@test_utils.test()
def test_args_hasher_data_oriented_with_opaque_member_silently_skipped() -> None:
    """Default ``@qd.data_oriented`` (no ``stable_members``) tolerates opaque-typed members.

    Opaque members (custom Python types not in the recognised set — UUIDs, plain Python objects,
    references to non-data-oriented classes, ...) are silently skipped from the fastcache key, because
    the kernel cannot read non-recognised Python types and therefore opaque members cannot affect
    kernel codegen. Pinning this here so we don't regress to silently disabling fastcache for any
    data_oriented holding metadata.
    """
    ...
    @qd.data_oriented
    class Container:
        def __init__(self, opaque: Opaque) -> None:
            self.meta = opaque
            self.nd = qd.ndarray(qd.i32, shape=(4,))

    c1 = Container(Opaque(1))
    c2 = Container(Opaque(2))
    h1 = args_hasher.hash_args(False, [c1], [None])
    h2 = args_hasher.hash_args(False, [c2], [None])
    assert isinstance(h1, str), f"opaque meta member must not disable fastcache, got {h1!r}"
    assert isinstance(h2, str)
    assert h1 == h2, "opaque-typed members must not affect the fastcache key"


@test_utils.test()
def test_args_hasher_data_oriented_nested_field_still_fails() -> None:
    """Recognised-but-unsupported (``qd.field``) inside a data_oriented container still disables fastcache.

    The opaque-member-silent-skip default does NOT relax this. Fields are *recognised* tensor-like types
    whose shape/dtype would affect kernel codegen, but fastcache doesn't yet support hashing them; the
    safe default is to disable fastcache for the whole call when one is encountered.
    """

    @qd.data_oriented
    class Container:
        def __init__(self) -> None:
            self.f = qd.field(qd.i32, shape=(4,))

    c = Container()
    h = args_hasher.hash_args(False, [c], [None])
    assert isinstance(h, FastcacheSkip), f"field inside data_oriented must disable fastcache, got {h!r}"


@test_utils.test()
def test_args_hasher_dataclass_with_opaque_field_silently_skipped() -> None:
    """A dataclass with an opaque-typed field should not disable fastcache."""
    ...
    class State:
        meta: object
        nd: qd.types.NDArray[qd.i32, 1]

    s1 = State(meta=Opaque(1), nd=qd.ndarray(qd.i32, (4,)))
    s2 = State(meta=Opaque(2), nd=qd.ndarray(qd.i32, (4,)))
    h1 = args_hasher.hash_args(False, [s1], [None])
    h2 = args_hasher.hash_args(False, [s2], [None])
    assert isinstance(h1, str), f"opaque dataclass field must not disable fastcache, got {h1!r}"
    assert h1 == h2, "opaque-typed fields must not affect the fastcache key"
