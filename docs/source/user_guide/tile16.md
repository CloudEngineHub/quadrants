...
The `dtype` argument is required. The underlying tile dataclass has 16 fields (`r0`–`r15`) — the scalar registers for one row. Tile types are cached per dtype internally.
