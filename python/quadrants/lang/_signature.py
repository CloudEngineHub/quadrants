...
def get_func_signature(func: Callable) -> inspect.Signature:
    ...
    Annotation-evaluation failures (``NameError``, ``AttributeError``) are
    re-raised as :class:`QuadrantsSyntaxError` with the offending function's
    qualified name, so users get a Quadrants-flavored error rather than a raw
    ``inspect`` traceback.
    """
    try:
